...
export default function ProductDetails() {
  ...
  return <>{loading ? <Loading /> : <> <div className='flex  justify-between w-10/12 m-auto gap-7 my-5'>
    <div className='w-5/12 flex flex-col justify-around '>
      <div className='shadow-xl rounded-md '>
        {myProduct?.images.length > 1 ? <Slider className='mb-8' {...settings}>
          {myProduct?.images.map((image) => <img src={image} className='w-full m-auto h-80 object-contain ' alt="Product Images" />)}
        </Slider> : <img src={myProduct?.images} className='w-full m-auto h-80 object-contain' alt="Product Images" />}
      </div>
    ...
    <div className='w-8/12 h-full bg-[#F8F9FA] rounded-md p-5'>
      ...
      <div className='flex justify-between items-center'>
        ...
        <div>
          <img className='w-full object-contain h-44' src={myProduct?.meta.qrCode} alt="" />
        </div>
      </div>
      <div className='flex items-center gap-3  py-3 border-t-2 border-b-2 border-gray-300  '>
        <div><h6 className='text-[#212529]'>Brand :</h6></div>
        <div className='font-bold'>{myProduct?.brand}</div>
      </div>
      <div className='flex items-center gap-3 border-b-2 border-gray-300 py-3 '>
        <div><h6 className='text-[#212529]'>Price :</h6></div>
        <div className=' flex items-center gap-3 '>
          <h6 className='text-red-600 line-through'>${myProduct?.price}</h6>
          <PriceCalc product={myProduct} />
          <div className='px-3 py-1 rounded-md bg-[#D1E7DD] text-[#198754] font-bold'>{myProduct?.discountPercentage}% off</div>
        </div>

      </div>
      <div className='flex items-center gap-3 border-b-2 border-gray-300 py-3 '>
        <div><h6 className='text-[#212529]'>Shipping :</h6></div>
        <div className='font-bold'>{myProduct?.shippingInformation}</div>
      </div>
      <div className='flex items-center gap-3 border-b-2 border-gray-300 py-3 '>
        <div><h6 className='text-[#212529]'>Availability :</h6></div>
        <div className='font-bold'>{`(${myProduct?.stock}) items Left ${myProduct?.availabilityStatus}  `}</div>
      </div>
      <div className='flex items-center gap-3 border-b-2 border-gray-300 py-3 '>
        <div><h6 className='text-[#212529]'>Category :</h6></div>
        <div className='font-bold'>{myProduct?.category}</div>
      </div>
      <div className='flex items-center gap-3 border-b-2 border-gray-300 py-3 '>
        <div><h6 className='text-[#212529]'>Return :</h6></div>
        <div className='font-bold'>{myProduct?.returnPolicy}</div>
      </div>
    </div>
  </div>
    <div className='w-10/12 m-auto mb-10 '>
      <h3 className='font-bold text-[#198754] text-4xl mx-10 py-5'>Reviews :</h3>
      <div className='flex justify-center flex-col gap-3  py-3 '>
        {myProduct?.reviews.length > 0 ? myProduct?.reviews.map((review) => <div className='bg-[#F8F9FA] px-3 py-8 rounded-md shadow-md w-5/12 border-2 border-gray-300'>
